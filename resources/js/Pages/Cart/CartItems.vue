...
watch(() => items, debounce(
    (value) => {
        value.qty>0 ? router.post('/cart-update',{id:value.id,qty:value.qty},{preserveScroll:true}):'';
    }
    , 500),{deep:true});
...
<template>
    <div class="row ps-3 py-2 align-items-center gap-2">
        ...
        <div class="col-4 d-flex gap-1 align-items-center">Quantity:<input type="number" :min="1" v-model="items.qty"
